...
### 재측정 (크레딧 충전 후, 리플레이 N = 8938d0f + c9c6edd + 85ddbae, 실제 LLM, 로컬 sqlite)
| 항목 | J(4번) | N |
|---|---|---|
| 리드 문장 수 분포 (프로브 턴) | {0: 8, 1: 51, 2: 7} | **{0: 6, 1: 60}** — 2문장 이상 0, 중앙값 1 |
| 되받기 시작 | 67 | 66 (허용 — 한 문장) |
| 연결 절 | 5 | 3 (LLM 자발) — 템플릿 인용 오류 0 |
| 하드 교정 | 9 | 16 (리드 상한 lead_cap 7 포함) |
| 또 묻기 / 질문 없는 턴 / 같은 질문 연속 | 1 / 0 / 0 | 3 / 0 / 0 |
| 앵커 커버리지 | 25/25 | 25/25 |
| SQL/턴 중앙값 | (41, 리플레이 L) | **11 (최대 13)** |
| 비LLM(total−llm) 중앙값 | — | 0.46s (로컬) |
| LLM 중앙값 | 1.6s | 3.0s (API 변동) |
