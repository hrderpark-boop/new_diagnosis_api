...
def build_intro_anchor_section() -> str:
    ...
    5개 영역 안내 + 시간 + 진단 방식 + 이해 확인.
    LLM 이 생성하는 호응 부분(사용자 기대 답변 paraphrase)은 별도로 앞에 붙음.
    """
    return (
        "본격적으로 시작하기 전에 진단에 대해 잠깐 안내드리겠습니다.\n\n"
        "오늘 함께할 진단은 5개 영역의 리더십 진단입니다. "
        "조직관리, 성과관리, 사람관리, 일관리, 자기관리 — "
        "이 다섯 가지 영역을 차례로 살펴볼 예정입니다.\n\n"
        "각 영역마다 약 30분 정도 소요되며, 전체적으로는 2-3시간 "
        "정도가 걸립니다. 한 번에 모두 진행하지 않으셔도 괜찮으며, "
        "중간에 저장하고 나중에 다시 이어가실 수 있습니다.\n\n"
        "진단 방식은 평가가 아니라 리더님의 실제 경험을 바탕으로 "
        "자연스럽게 대화를 나누는 형식입니다. 정답이 정해져 있지 "
        "않으니 편하게 말씀해주시면 됩니다.\n\n"
        "이렇게 진행될 예정입니다. 이해되셨을까요?"
    )
