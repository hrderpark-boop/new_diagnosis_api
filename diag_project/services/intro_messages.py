...
def build_intro_anchor_section() -> str:
    """진단 안내 본문 (구조화 버전 — 5개 영역 + 소요 시간 + 형식).

    하이브리드 INTRO 흐름에서 시스템이 직접 출력하는 부분.
    LLM 이 생성하는 호응 1문장 뒤에 이어 붙음.
    """
    return (
        "그럼 오늘 진단이 어떻게 진행되는지 간단히 안내드릴게요.\n\n"
        "진단은 5개 영역으로 이루어져 있어요 — 조직관리, 성과관리, "
        "사람관리, 일관리, 자기관리. 각 영역마다 약 30분 정도, "
        "리더님의 실제 경험을 바탕으로 대화하듯 이야기 나눕니다.\n\n"
        "평가가 아니라 함께 돌아보는 시간이니 정답 걱정 없이 편하게 "
        "말씀해 주시면 돼요. 중간에 저장하고 나중에 이어가셔도 "
        "괜찮습니다.\n\n"
        "그럼 시작해볼까요?"
    )
