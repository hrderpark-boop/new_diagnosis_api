...
# 2026-09-21: "신뢰" 삭제 — 사람관리(신뢰형성)에서 '신뢰'가 든 답변 14턴이 전부 메타 경로로 빠졌다.
#   "이 평가" 도 '~이 평가~' 부분 일치를 냈다 → "이 평가는/이 평가가/이 평가를" 로 좁힘.
META_KEYWORDS = [
    "AI가", "당신이", "이 시스템",
    "정확한가요", "맞나요", "근거가",
    "어떻게 평가", "평가 방식", "이거 믿",
    "AI는", "AI랑", "이 평가는", "이 평가가", "이 평가를",
]
# 메타 질문·항의는 짧은 발화다. 이 길이를 넘는 서술형 답변(사례 서술)은 키워드가 섞여도 메타로 보지 않는다.
META_MAX_LEN = 120
...
ABSENCE_KEYWORDS = [
    ...
    "위임하지 않", "위임한 적이 없", "위임한 적은 없", "맡기지 않", "맡긴 적이 없", "맡긴 적은 없",
    # 2026-09-17: "특별한 것 없음 / 그때그때 / 딱히" 계열
    "특별한 것 없", "특별한 것은 없", "특별한 건 없", "특별한 게 없", "특별한 일은 없", "특별한 사례",
    "딱히 기억", "딱히 떠오르", "딱히 특별", "별건 없", "별거 없", "없었던 것 같", "그때그때 해결", "그때그때 처리",
]
...
_SHORT_ABSENCE_ENDINGS = ("없었는데", "없었어요", "없었습니다", "없네요", "없어요",
                          "없습니다", "없는데요", "없는 것 같", "그때그때", "딱히", "특별한 것 없음", "없음")
...
def detect_meta_question(text: str | None) -> bool:
    """시스템에 대한 메타 질문 감지 — 짧은 발화(≤META_MAX_LEN)에서만."""
    if not text:
        return False
    t = text.strip()
    if len(t) > META_MAX_LEN and "?" not in t:
        return False
    return any(kw in t for kw in META_KEYWORDS)
