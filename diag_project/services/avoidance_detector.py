...
#  ⚠️ 부분 문자열 매칭이므로 '길고 성실한 발화 속에 우연히 섞일 수 있는'
#     모호한 조각(예: "위에서"→"하늘 위로", "의미가 있"→"의미가 있는지 없는지")
#     은 넣지 않는다. 그런 오탐이 협조적·회의적 대상자를 강제 종료시켰다.
DEFLECTION_KEYWORDS = [
    # 남탓 / 외부 귀인
    "무능", "시스템이 엉망", "시스템 문제", "회사가 문제", "회사 탓",
    "팀원들 때문", "팀원이 문제", "쟤네가", "걔네가", "위에서 시켜",
    "제 잘못이 아", "내 잘못이 아", "환경이 안",
    # 비아냥 / 진단 무시 / 도발 / 불만·비하
    "왜 물어", "왜 해요", "뭐가 달라", "뭐가 바뀌",
    "알기나 해", "네가 뭘", "당신이 뭘", "이런 거 해서",
    "쓸데없", "시간 낭비", "그래서 뭐", "관심 없",
    "이딴", "이따위", "됐고", "됐거든", "됐어요", "치워",
    # 귀찮: 불만형 어미만 (부정문 "귀찮은 게 아니라"는 제외되게 "귀찮은"은 뺌)
    "귀찮아", "귀찮게", "귀찮다", "귀찮네", "귀찮으",
    "뭐 하러", "뭐하러", "은 무슨", "는 무슨", "무슨 소용",
    "하기 싫", "안 할래", "왜 이런",
]

# 단일 표지만 걸렸을 때 '퉁명스러운 도발'로 인정하는 최대 길이(자).
#  이보다 긴 발화 속 단일 조각 매칭은 오탐으로 보고 도발로 세지 않는다.
SHORT_DEFLECTION_LEN = 35


def detect_deflection(text: str | None) -> bool:
    """남탓·비아냥·도발 감지 (유효 데이터 회피의 또 다른 형태).

    오탐(협조적·회의적 대상자의 강제 종료) 방지를 위해 다음만 도발로 본다:
      · 도발 표지가 2개 이상 겹치면 → 길이와 무관하게 도발 확정
      · 표지가 1개뿐이면 → 짧고 퉁명스러운 발화(SHORT_DEFLECTION_LEN 이하)만
    길고 구체적인 발화 속 한 조각 매칭(예: "의미가 있는지 없는지…")은 제외한다.
    """
    if not text:
        return False
    stripped = text.strip()
    hits = {kw for kw in DEFLECTION_KEYWORDS if kw in stripped}
    if len(hits) >= 2:
        return True
    if len(hits) == 1:
        return len(stripped) <= SHORT_DEFLECTION_LEN
    return False
