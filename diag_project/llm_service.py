...
# Gap Analysis 고정 접두 문자열 — 리포트 톤 통일용 (프롬프트 + 백엔드 이중 강제)
GAP_ANALYSIS_PREFIX = "향후 이상적인 리더십 역량 개발을 위해서는"


def _sanitize_gap_analysis(text: str) -> str:
    """Gap Analysis 텍스트 강제 정제 (LLM 프롬프트 무시 대비 하드 가드).

    1. '5.0 만점/만점 수준/점수 도달' 류 금지 표현을 제거·치환
    2. 첫 문장이 반드시 GAP_ANALYSIS_PREFIX 로 시작하도록 덮어씀
    """
    import re
    ...
    # 2) 고정 접두 강제 (유사 접두가 이미 있으면 정규화 후 재부착)
    if not t.startswith(GAP_ANALYSIS_PREFIX):
        t = re.sub(
            r"^(향후\s*)?(이상적인\s*리더십(\s*역량)?\s*개발(을|과)?"
            r"\s*(위해서는|위해|위하여)?)[,\s]*",
            "", t,
        ).strip()
        if t:
            t = f"{GAP_ANALYSIS_PREFIX} {t}"
        else:
            t = (f"{GAP_ANALYSIS_PREFIX} 구체적인 현업 실행 경험을 "
                 "꾸준히 축적하는 것이 필요합니다.")
    return t
...
{{
  ...
  "top_keywords": [
    {{"keyword": "이 리더를 대표하는 키워드1", "meaning": "이 키워드가 이 리더의 비즈니스/조직 운영에서 갖는 의미를 단문 1문장으로"}},
    {{"keyword": "키워드2", "meaning": "비즈니스적 의미 1문장"}},
    {{"keyword": "키워드3", "meaning": "비즈니스적 의미 1문장"}},
    {{"keyword": "키워드4", "meaning": "비즈니스적 의미 1문장"}},
    {{"keyword": "키워드5", "meaning": "비즈니스적 의미 1문장"}}
  ]
}}
"""
        try:
