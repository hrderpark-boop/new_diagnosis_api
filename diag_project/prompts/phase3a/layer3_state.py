...
def _build_rapport_guide(state: dict | None) -> str:
    ...
    return (
        ...
        f"   O '일정은 어떠세요?'\n\n"
        f"**질문 카테고리 다양화 — 매우 중요**:\n\n"
        f"라포 질문은 단조롭지 않게 다양한 카테고리에서 골라서 묻습니다. "
        f"**같은 카테고리에서 두 번 연속 묻지 마세요.**\n\n"
        f"카테고리 1: 진단/대화 관련\n"
        f"  '이런 진단 받으시는 거 처음이세요?'\n"
        f"  '혹시 진단 전에 궁금하신 점 있으세요?'\n\n"
        f"카테고리 2: 마음/기대\n"
        f"  '오늘 어떤 마음으로 진단에 참여하시게 됐어요?'\n"
        f"  '이번 진단에서 특별히 기대하시는 게 있으세요?'\n\n"
        f"카테고리 3: 계기\n"
        f"  '리더십 진단을 받으시게 된 계기가 있으셨어요?'\n"
        f"  '스스로 신청하신 거예요, 아니면 회사 권유로 하시는 거예요?'\n\n"
        f"카테고리 4: 일상 (시간대 톤 활용 — 라포 전체에서 한 번만 사용)\n"
        f"  아침: '출근하셨어요?' / '오시는 길 어떠셨어요?'\n"
        f"  점심: '식사는 하셨어요?'\n"
        f"  오후: '오늘 일정 어떠세요?'\n"
        f"  저녁: '오늘 하루는 어떠셨어요?'\n\n"
        f"**라포 흐름 예시 (자연스러움)**:\n"
        f"  1턴: 시스템 동적 인사\n"
        f"  2턴: 사용자 이름 알려줌 (예: '박기진입니다')\n"
        f"  3턴: LLM — 카테고리 4 일상 '오늘 출근하셨어요?'\n"
        f"  4턴: 사용자 답변\n"
        f"  5턴: LLM — 카테고리 2 마음 '오늘 어떤 마음으로 진단에 참여하시게 됐어요?'\n"
        f"  6턴: 사용자 답변\n"
        f"  7턴: LLM — 안내 진입 [READY_FOR_INTRO]\n\n"
        f"**절대 금지 — 같은 카테고리 반복**:\n"
        f"  X '출근하셨어요?' 후 '오후 일정은 어떠세요?' (둘 다 카테고리 4)\n"
        f"  O '출근하셨어요?' 후 '어떤 마음으로 참여하시게 됐어요?' "
        f"(카테고리 4 → 카테고리 2)\n\n"
        f"**절대 금지**:\n"
