...
def _get_instruction_guide(
    ...
    if _next_name:
        ...
        # 🏁 마지막 영역(자기관리) — 다음 영역 없음. Grand Wrap-up 강제.
        _ready_to_end_guide = (
            "🏁 **[FINAL CHAPTER — 진단 전체의 마지막입니다]**\n"
            "현재 리더십 진단의 **마지막 5번째 역량 탐색까지 모두 종료**됐습니다. "
            "이 뒤에 이어질 다음 영역은 없습니다.\n\n"
            "🚫 **절대 금지**: '다음 영역/챕터로 넘어가자', '이어서 ~를 "
            "살펴볼까요?', '다음은 ~입니다' 등 **다음을 예고·암시하는 어떤 "
            "표현도 절대 출력하지 마세요.** (다음은 존재하지 않습니다)\n\n"
            "✅ **해야 할 일 — Grand Wrap-up (3~4문장)**:\n"
            "150분가량 진행된 전체 진단 여정에 끝까지 몰입하고 깊이 성찰해 "
            "주신 리더님의 노고에 진심 어린 감사와 경의를 표하세요. 오늘 "
            "다섯 영역을 관통하며 드러난 리더님의 묵직한 결을 한 번 더 "
            "따뜻하게 비춰주고, 곧 정리될 진단 결과에 대한 기대를 담아 "
            "임원 코치의 품격에 맞게 묵직하고 감동적으로 마무리하세요.\n\n"
            "(종료 마커는 시스템이 부착하므로 태그는 쓰지 마세요.)"
        )
