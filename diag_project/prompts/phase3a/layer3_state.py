...
def _get_instruction_guide(
    ...
    guides = {
        ...
        "COMPETENCY_ALIGN": (
            ...
            "사용자가 말한 핵심 내용을 짧게 되짚으며 공감으로 연결하세요.\n"
            "  필수 형식: '네, 리더님께서 [사용자가 말한 핵심]라고 하셨는데,'\n"
            "  → 이 뒤에 시스템이 '리더님 말씀을 포함해서 ...라고 정의합니다'로 "
            "자연스럽게 이어집니다.\n"
            "  예 (사용자: '조직을 효율적으로 관리하는 것'):\n"
            "   '네, 리더님께서 조직을 효율적으로 끌어가는 것이라고 "
            "하셨는데, 한정된 자원으로 성과를 내려는 균형 감각이 "
            "느껴지네요.'\n\n"
            "**[분기 B] 무성의·회피·짧은 답변** "
