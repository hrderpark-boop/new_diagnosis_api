...
def _get_instruction_guide(
    ...
    if _next_name and _no_yield_forced:
        ...
        _ready_to_end_guide = (
            ...
            "영역을 매듭지으세요. (매번 다른 톤)\n\n"
            f"② **강제 전환 선언 + 다음 역량 도입 (필수)**: 요약 뒤에 곧바로, "
            "선택을 묻지 말고 다음 역량으로 넘어감을 '선언'하세요. 결 참고"
            "(그대로 복붙 금지, 매번 변주):\n"
            f"   '리더님의 귀한 경험 잘 들었습니다. {_cur_chapter_name} 영역은 "
            f"충분히 담겼으니, 바로 다음 역량인 {_next_name}{get_josa(_next_name, '으로/로')} "
            "이어가 보겠습니다.'\n"
            f"   → 그리고 자연스럽게 {_next_name}에 대한 리더님의 생각을 여는 "
            "첫 질문 하나까지 이어서 던지세요 (한 턴에 종결+전환+첫 질문).\n\n"
            "🚨 **절대 금지**:\n"
            ...
            f"이 영역은 충분히 담겼으니, 바로 다음 {_next_name} 이야기로 "
            f"이어가 보겠습니다 — {_next_name}와 관련해, 최근 리더님께서 "
            "가장 신경 쓰셨던 장면을 하나 떠올려 주실 수 있을까요?'"
        )
    else:
    ...
    guides = {
        ...
        "COMPETENCY_ALIGN": (
            f"역량 수용+정의 제시(Step 6) — '{_cur_chapter_name}' 영역. 직전 턴에 "
            "리더님께 '평소 생각'을 여쭸고, 지금은 그 답을 '포괄'하며 공식 정의와 "
            "하위역량을 안내하는 턴입니다.\n\n"
            "🚨 감정 상태 점검: 직전 답변이 극심한 스트레스·거부면 역량 이야기 "
            "말고 감정에 머무르세요(Core Rule 7).\n\n"
            "다음 3가지를 자연스러운 대화체로 엮으세요:\n"
            "1) **리더님 답변 포괄(부정 금지)**: 방금 리더님이 말한 정의를 "
            "'그것도 맞고, 포함된다'는 태도로 끌어안으세요. 부정·교정 금지. "
            "(무성의·짧은 답이면 '한마디로 정의하긴 쉽지 않으시죠, 괜찮습니다'로 "
            "부담을 덜기.) 예: '말씀하신 ~도 그 안에 담기고, 저희는 거기에 …까지 "
            "포함해서 봅니다.'\n"
            f"2) **공식 정의 + 하위역량 목록 안내(필수)**: 이번 역량을 대화체로 "
            f"풀되, **하위역량 {_sub_count_ko} 가지를 실제로 안내**하세요. "
            f"competencies.py 기준 목록(그대로): {_sub_plain}.\n"
            f"   예: '저희 진단에서 {_cur_chapter_name}은(는) {_sub_plain} "
            f"{_sub_count_ko} 가지로 나눠 살펴봅니다.'\n"
            f"   · 공식 정의(녹여쓰기용): '{_cur_def}'\n"
            "   · 목록은 competencies.py 값 그대로. 임의 추가·생략·왜곡 금지.\n"
            "3) **탐색 전환 예고**: 곧 실제 경험을 들여다볼 것임을 한 문장으로.\n\n"
            "⛔ 금지: 정의 문장 통째 교과서 인용 / 딱딱한 '동의하십니까?' / "
        ...
        "CHAPTER_NO_YIELD_ULTIMATUM": (
            "🛑 **최후통첩 — 여러 턴째 구체적 실사례가 안 나오고 있음.**\n\n"
            "지금까지 질문을 여러 번 던졌지만 리더님은 계속 원론·개념 수준에 "
            "머물러, 채점 가능한 실제 경험(STAR)이 하나도 확보되지 않았습니다. "
            "이 역량은 이대로면 '미달'로 기록됩니다.\n\n"
            "단호하지만 정중하게 '마지막 한 번'을 요청하세요:\n"
            "  '리더님, 계속해서 구체적인 실제 사례를 파악하기가 어렵네요. "
            "이대로면 이 영역은 제대로 담아드리기 어려워요. 마지막으로, "
            "현업에서 리더님이 직접 행동하신 사례 딱 한 가지만 여쭐게요 — "
            "언제, 어떤 상황에서, 리더님이 구체적으로 무엇을 하셨는지 "
            "짧게라도 들려주시겠어요?'\n\n"
            "🚫 새 이론 질문 금지. 이것이 이 역량의 마지막 기회임을 "
            "분명히 하되 비난조는 피하세요. 한 번만 요청하고 답을 기다리세요."
        ),
