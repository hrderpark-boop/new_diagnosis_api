...
def _get_instruction_guide(
    ...
    guides = {
        ...
        "COMPETENCY_ALIGN": (
            f"역량 수용(Step 6) — '{_cur_chapter_name}' 영역으로 자연스럽게 "
            "들어가는 턴입니다. ⚠️ 시스템이 뒤에 붙이던 '공식 정의 + 하위역량 "
            "목록 + 동의 질문' 기계 텍스트는 폐지됐습니다. 이제 이 턴의 응답을 "
            "당신이 온전히, 대화체로 직접 완성하세요.\n\n"
            "🚨 **[최우선 분기 — 감정 상태 점검]**: 직전 답변이 '극심한 "
            "스트레스·좌절·진단 거부'라면 역량 이야기를 절대 하지 말고, 그 "
            "감정에 머무르며 (Core Rule 7) 필요시 [SUGGEST_PAUSE]로 끝내세요.\n\n"
            "다음 3가지를 자연스러운 2-4문장으로 엮으세요:\n"
            "1) **리더님의 답변 반영**: 방금 리더님이 들려주신 평소 생각/정의의 "
            "핵심을 한 조각 되짚어 공감. (거짓 공감·환각 금지. 무성의·회피·짧은 "
            "답이면 억지로 되짚지 말고 '한마디로 정의하긴 쉽지 않으시죠, "
            "괜찮습니다'로 부담을 덜어주기.)\n"
            "2) **정의 녹여내기(변수 주입)**: 아래 공식 정의를 교과서처럼 "
            "인용하지 말고, 리더님 말과 '같은 결'로 대화체로 풀어 연결하세요.\n"
            f"   · 이번 역량 정의: '{_cur_def}'\n"
            f"   · (참고용 하위 결: {_sub_list_str} — 절대 목록으로 나열하지 "
            "말 것. 스코프 감만 잡고, 필요하면 1개 정도만 자연스레 언급)\n"
            "3) **탐색으로의 부드러운 전환**: 곧 실제 경험을 들여다볼 것임을 "
            "한 문장으로 예고하며 마무리. (구체 BEI 질문 자체는 다음 턴 담당)\n\n"
            "**⛔ 절대 금지**:\n"
            "- '이 역량의 하위 역량은 다음 N가지입니다: -,-,-,-' 같은 목록 나열\n"
            "- 정의 문장 통째 인용 / 딱딱한 '동의하십니까?' 식 확인 질문\n"
            "- 없는 답변을 있는 것처럼 되짚는 거짓 공감\n\n"
            "톤: 페르소나를 살려 따뜻하고 자연스럽게. 4문장 이내."
        ),
