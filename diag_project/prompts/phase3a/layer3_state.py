...
def _build_confirm_guide(state: dict | None) -> str:
    ...
    if is_first_chapter:
        ...
        opening_strategy = (
            f"**브리지 + 정의 질문 (전환 영역)**:\n"
            f"앞 영역에서 리더님이 보여준 강점·이야기를 한 줄로 짚어 다리를 "
            f"놓은 뒤, '{chapter_name}'에 대한 명확한 정의 질문을 던지세요. "
            f"(대화 이력 참고)\n\n"
            f"  좋은 예시:\n"
            f"    '앞서 보여주신 리더십이 참 인상 깊었어요. 그 결을 이어서, "
            f"리더님께서는 '{chapter_name}'을 무엇이라고 생각하시나요? "
            f"그렇게 생각하시는 이유도 간단히 들려주세요. [START_CHAPTER]'\n\n"
        )
    ...
    return (
        ...
        f"**긍정인 경우 — 도입 + 정의 질문 (한 응답)**:\n\n"
        f"{opening_strategy}"
        f"  → [START_CHAPTER] 태그 필수.\n"
        f"  → 질문은 반드시 '무엇이라고 생각하시나요? + 그 이유' 구조로 "
        f"명확하게. ('떠오르는 장면' 식 막연한 질문 금지)\n\n"
        f"⛔ 절대 금지:\n"
