...
def _get_instruction_guide(
    ...
    if _next_name and _no_yield_forced:
        ...
        _ready_to_end_guide = (
            "🛑 **[무수확 종료] 이 영역에서는 채점 가능한 구체적 실사례가 "
            "끝내 확보되지 않았습니다.** 없는 강점을 지어내 칭찬하지 마세요 "
            "(거짓 공감 금지).\n\n"
            "담백하고 정중하게 이 영역을 정리하고, 다음 영역으로 넘어갈지 "
            "'계속/휴식'을 여쭤보며 응답을 끝내세요:\n"
            "  예: '이 영역은 구체적인 사례까지는 오늘 충분히 담기 어려웠네요. "
            "그건 그것대로 괜찮습니다. 이 부분은 여기서 정리하고, 다음 "
            f"'{_next_name}' 이야기로 이어가 볼까요, 아니면 잠시 쉬었다 "
            "갈까요?'\n\n"
            "🚫 없는 경험을 있는 것처럼 요약하거나 과장된 강점 칭찬 금지. "
            "[CHAPTER_COMPLETE]/[START_CHAPTER] 태그는 시스템이 부착합니다."
        )
    elif _next_name:
        _ready_to_end_guide = (
            "이 영역의 조건이 모두 충족됐습니다. 새 BEI 탐침은 던지지 마세요.\n\n"
            f"🛡️ **[매우 중요] 아직 다룰 역량이 남아있습니다 (다음: "
            f"'{_next_name}' 외).** 이것은 '한 영역'의 마무리일 뿐, 절대 "
            "'전체 진단의 종료'가 아닙니다. 전체 종료·작별 인사·'모든 진단이 "
            "끝났다'는 멘트, [DIAGNOSIS_COMPLETE] 태그를 **절대 쓰지 마세요.**\n\n"
    ...
    guides = {
        ...
        "COMPETENCY_ALIGN": (
            ...
            "🚨 **[최우선 분기 — 감정 상태 점검]**: 직전 답변이 역량 정의가 "
            "아니라 '극심한 스트레스·좌절·진단 거부'라면, 역량 호응·정의 "
            "이야기를 절대 하지 마세요. 그 감정에 온전히 머무르고 (Core "
            "Rule 7) 필요하면 [SUGGEST_PAUSE]로 쉬어갈지 제안하며 응답을 "
            "끝내세요. 이 경우 시스템도 정의 텍스트를 붙이지 않습니다.\n\n"
        ...
        "STAR_COMPLETE_NEW_EVENT": (
            ...
            "이름을 그대로 읊지 마세요.\n\n"
            "🚨 **절대 금지 (기계적 화제 전환)**:\n"
            "- '다음으로 ~에 대해 여쭤볼게요' / '이번엔 ~ 역량을 살펴볼게요' "
            "처럼 진단 항목을 갈아끼우듯 딱딱하게 넘어가기.\n"
            f"- 하위 역량 이름('{_first_unexplored}' 등)을 교과서처럼 그대로 "
            "호명하며 질문 던지기.\n"
