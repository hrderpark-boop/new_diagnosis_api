...
def _build_confirm_guide(state: dict | None) -> str:
    ...
    # 챕터별 도입 전략: 첫 영역은 따뜻한 초대, 이후 영역은 직전 대화와 브리지
    if is_first_chapter:
        opening_strategy = (
            f"**도입 방식 (첫 영역) — 판에 박힌 질문 금지**:\n"
            f"'{chapter_name}가 무엇이라고 생각하세요?' 같은 사전식 질문은 "
            f"딱딱하고 부담스럽습니다. 대신 리더님의 머릿속에 그림이 "
            f"떠오르도록 환기하는 초대를 건네세요.\n\n"
            f"  좋은 예시 1:\n"
            f"    '준비되셨다니 좋습니다. 그럼 첫 영역인 '{chapter_name}'부터 "
            f"함께 들여다볼게요. 리더님께 '{chapter_name}' 하면 가장 먼저 "
            f"떠오르는 장면이나 생각이 있으세요? 편하게 들려주세요. "
            f"[START_CHAPTER]'\n\n"
            f"  좋은 예시 2:\n"
            f"    '좋습니다, 바로 시작해볼게요. 리더님은 평소 '{chapter_name}'를 "
            f"잘한다는 게 어떤 모습이라고 느끼세요? 정답이 있는 게 아니니 "
            f"떠오르는 대로 말씀해 주시면 돼요. [START_CHAPTER]'\n\n"
        )
    else:
        opening_strategy = (
            f"**도입 방식 (전환 영역) — 직전 대화와 브리지(Bridge)**:\n"
            f"화제를 뚝 끊고 '{chapter_name}가 무엇이라고 생각하세요?'라고 "
            f"새로 묻지 마세요. 앞 영역에서 리더님이 보여준 강점·이야기를 "
            f"한 줄로 짚은 뒤, 그것이 '{chapter_name}'와 어떻게 연결되는지 "
            f"자연스럽게 다리를 놓으며 넘어가세요. (대화 이력 참고)\n\n"
            f"  좋은 예시 1:\n"
            f"    '앞서 사람을 대하는 리더님의 철학이 참 인상 깊었어요. "
            f"그 결이 '{chapter_name}'로 이어질 때는 또 어떤 모습이 될지 "
            f"궁금해지는데요. 리더님께 '{chapter_name}'는 어떤 의미로 "
            f"다가오세요? [START_CHAPTER]'\n\n"
            f"  좋은 예시 2:\n"
            f"    '방금 들려주신 이야기 덕분에 자연스럽게 '{chapter_name}' "
            f"이야기가 떠올랐어요. 리더님은 이 영역에서 가장 중요한 게 "
            f"뭐라고 보세요? [START_CHAPTER]'\n\n"
        )
    ...
    return (
        ...
        f"**긍정인 경우 — 도입 + 생각 묻기 (한 응답)**:\n\n"
        f"{opening_strategy}"
        f"  → [START_CHAPTER] 태그 필수.\n\n"
        f"⛔ 절대 금지:\n"
        f"  X 준비 인사만 하고 끝내기 ('준비됐다니 좋습니다.' 만 X)\n"
        f"  X 질문 없이 [START_CHAPTER] 보내기\n"
        f"  X BEI 질문 (사건/경험 묻기) — 생각·관점만 묻기\n"
        f"  X '{chapter_name}가 무엇이라고 생각하세요?' 식 사전형 질문\n"
...
def _get_instruction_guide(
    ...
    guides = {
        ...
        "COMPETENCY_ALIGN": (
            ...
            "공식 정의 + 하위역량 4가지를 자동으로 이어 붙입니다. "
            "따라서 당신은 1-2문장 호응만 출력하고 끝내세요.\n\n"
            "🚨 **가장 중요 — 직전 사용자 답변의 '실제 내용'을 보고 분기**:\n"
            "방금 사용자가 한 답변을 그대로 보세요. 거짓 공감(환각) 절대 금지.\n\n"
            "**[분기 A] 사용자가 나름의 생각/정의를 말한 경우**:\n"
            "그 관점의 핵심 가치를 포착해 진짜로 공감 (1-2문장). "
            "기계적 복창('~라고 하셨군요') 말고 통찰 한마디로.\n"
            "  예 (사용자: '조직을 효율적으로 관리하는 것이요'):\n"
            "   '효율이라는 말 속에, 한정된 자원으로 최대 성과를 내려는 "
            "균형 감각이 느껴지네요.'\n\n"
            "**[분기 B] 무성의·회피·짧은 답변** "
            "('ㅡㅡ', '글쎄요', '잘 모르겠어요', '그냥요', 'ㅎㅎ', 무의미 입력):\n"
            "🚫 절대 '리더님 말씀이 저희 철학과 맞닿아 있네요' 같은 거짓 공감 X.\n"
            "→ 대신 부담을 덜어주고 유연하게 정의로 넘어가는 한마디:\n"
            "   '당장 한마디로 정의하긴 쉽지 않으시죠? 괜찮습니다. "
            "사실 현업에서는 보통 이렇게들 봅니다.'\n"
            "   '깊이 생각하게 되는 주제죠. 제가 기준점을 하나 드려볼게요.'\n\n"
            "**⛔ 공통 금지**:\n"
            "- 세부역량 목록·정의 문장 → 시스템 담당 (출력 X)\n"
            "- 합의 질문('진행해도 될까요?', '괜찮으시겠어요?')\n"
            "- BEI 질문(사건/경험 묻기) → 다음 턴 담당\n"
            "- 2문장 초과\n\n"
            "톤: 페르소나 살리되, 입력 품질에 맞는 호응 한마디만."
        ),
        ...
        "STAR_COMPLETE_NEW_EVENT": (
            "한 경험을 충분히 들었습니다. 그 이야기의 핵심을 한 줄로 "
            "따뜻하게 매듭짓고('그 결단이 지금의 팀을 만든 셈이네요'), "
            "자연스럽게 또 다른 장면으로 초대하세요. "
            "'혹시 결이 다른 또 다른 경험도 있으실까요?' 처럼 부담 없이. "
            "기계적인 '다음 사례를 말씀해 주세요' 금지."
        ),
        "CONTRARY_NEEDED": (
