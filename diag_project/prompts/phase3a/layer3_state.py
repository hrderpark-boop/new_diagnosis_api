...
def _get_instruction_guide(
    ...
    if _next_name:
        _ready_to_end_guide = (
            ...
            "**당신의 역할은 딱 하나 — 방금 영역의 'Wrap-up'만 작성**:\n"
            "방금 나눈 이야기에서 드러난 리더님의 강점·결을 짚어, 따뜻하고 "
            "진심 어린 요약·공감을 2~3문장으로 매듭지으세요. (매번 다른 톤)\n\n"
            "🚨 **하지 말 것 (시스템이 자동 처리함)**:\n"
            "- 다음 영역으로의 전환 질문 ❌ → 시스템이 덧붙임\n"
            "- '넘어갈까요?' 류 확인 질문 ❌\n"
            "- [CHAPTER_COMPLETE]/[START_CHAPTER] 태그 ❌ → 시스템이 부착\n"
            "- 새 질문 일절 금지. **wrap-up 문장으로 자연스럽게 끝맺기만** 하세요.\n\n"
            "예: '말씀을 들으며 리더님이 사람을 대하는 깊이가 느껴졌어요. "
            "쉽지 않은 상황을 그렇게 풀어내신 점이 오래 기억에 남을 것 같네요.'"
        )
    else:
