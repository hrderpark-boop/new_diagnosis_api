...
def _get_instruction_guide(
    ...
    if _next_name:
        _ready_to_end_guide = (
            ...
            "**대화를 뚝 끊지 말고, 매번 다른 톤으로 따뜻하게 매듭 + "
            f"다음 영역('{_next_name}')으로 자연스럽게 초대**하세요. "
            "고정된 '진단을 마쳤습니다. 넘어갈까요?' 멘트 금지 — "
            "방금 나눈 이야기의 여운을 살려 매번 새롭게 변주.\n\n"
            "**좋은 예시 (이 결로, 매번 다르게)**:\n"
            f"- '이 이야기 나누다 보니 시간 가는 줄 몰랐네요. 이 에너지 "
            f"그대로 '{_next_name}' 이야기도 들어보고 싶어요.'\n"
            f"- '방금 보여주신 모습이 '{_next_name}'에서는 또 어떻게 "
            f"드러날지 벌써 궁금해지는데요, 이어서 가볼까요?'\n"
            f"- '여기까지만 들어도 리더님의 결이 선명하네요. 자연스럽게 "
            f"'{_next_name}' 쪽으로 발걸음을 옮겨볼게요.'\n\n"
