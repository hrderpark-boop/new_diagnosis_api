...
def _build_rapport_guide(state: dict | None) -> str:
    ...
    category_guides = {
        ...
        "기대": (
            ...
            "← 이전 턴에서 이미 물음\n"
            "- 계기 캐묻기 ('회사 차원? 직접 신청?') ← 사용자가 답할 동기 없음\n"
            "- BEI 질문 (사건/경험)\n"
            "- 한 번에 두 개 이상 질문\n\n"
        ...
        "진단_대화": (
            ...
            "사용자가 본격 진단으로 넘어갈 준비가 됐는지 확인.\n"
            "이전 사용자 답변을 분석해서 아래 3가지 중 하나를 선택.\n\n"
            "**필수 행동 (이전 답변 분석)**:\n"
            "1. 시작 의지 표현 ('네 시작해주세요', '준비됐어요', '이제 시작하죠')\n"
            "   → [READY_FOR_INTRO] 즉시 보내기\n"
            "   예: '네, 그럼 진단 안내드릴게요. [READY_FOR_INTRO]'\n\n"
            "2. 망설임 표현 ('글쎄요', '잘 모르겠어요', '아무 생각 없어요')\n"
            "   → 가벼운 응대 + 시작 의지 확인\n"
            "   예: '편하게 진행하시면 됩니다. 진단 안내로 넘어가도 될까요?'\n\n"
            "3. 짧고 모호 ('네', '아니요')\n"
            "   → 시작 의지 명확히 확인\n"
            "   예: '그럼 진단 안내로 넘어가도 될까요?'\n\n"
            "**절대 금지**:\n"
            "- 이미 물어본 카테고리 (일상/기대) 질문 반복\n"
            "- 계기 캐묻기 ('회사 차원? 직접 신청?')\n"
            "- 사용자 동의 없이 [READY_FOR_INTRO] 보내기 (안전장치 위반)\n"
            "- BEI 질문\n"
            "- 한 번에 두 개 이상 질문"
        ),
    }
