...
def _get_instruction_guide(
    ...
    if _next_name:
        _ready_to_end_guide = (
            ...
            "매듭지으세요. (매번 다른 톤)\n\n"
            f"② **계속/휴식 의사 확인 질문 (필수)**: 요약·공감 뒤에는 반드시, "
            f"다음 역량인 '{_next_name}' 진단을 지금 바로 이어서 진행할지 "
            "아니면 잠시 숨을 돌렸다가 나중에 이어갈지, 리더님의 의사를 "
            "자연스럽게 여쭤보며 응답을 끝내세요.\n"
            "   🚨 **매번 똑같은 문장 절대 금지** — 맥락·톤에 맞게 표현을 매번 "
            "다르게 변주하세요.\n"
            "   결만 참고(그대로 복붙 금지): '한숨 돌리고 다음으로 넘어갈까요?', "
            f"'계속해서 {_next_name} 이야기를 살펴볼까요, 아니면 잠깐 쉬었다 "
            "갈까요?', '바로 이어가도 괜찮으실까요, 아니면 잠시 쉬어가실래요?'\n\n"
            "🚨 **하지 말 것**:\n"
            ...
            f"이제 {_next_name}로 바로 이어가 볼까요, 아니면 잠깐 숨 돌리고 "
            "가실까요?'"
        )
    else:
