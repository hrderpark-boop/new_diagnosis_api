...
def _get_instruction_guide(
    ...
    if _next_name and _no_yield_forced:
        # 무수확 강제 전환: 강점 요약이 아니라 '구체 사례 확보 실패'를 담백히
        # 인정하고 다음으로 넘어간다. (거짓 강점 칭찬 환각 방지)
        _ready_to_end_guide = (
            "🛑 **[무수확 강제 전환 — Fail-Fast] 이 영역에서는 채점 가능한 "
            "구체적 실사례(STAR)가 끝내 확보되지 않았습니다.** 회피·남탓·"
            "비아냥이 반복되었거나 개념만 맴돌아, 더 구슬려도 유의미한 데이터가 "
            "나올 가능성이 낮습니다. 없는 강점을 지어내 칭찬하지 마세요 "
            "(거짓 공감 금지).\n\n"
            "🎯 코치의 전문가적 판단으로, 구슬리기를 멈추고 이 영역을 단호하게 "
            "정리한 뒤 다음 역량으로 '강제 전환'을 선언하세요. '계속/휴식'을 "
    ...
    guides = {
        ...
        "COMPETENCY_ALIGN": (
            ...
            "표현으로 되돌아오며 닫으세요.\n"
            "3) **탐색 전환 예고**: 곧 실제 경험을 들여다볼 것임을 한 문장으로.\n\n"
