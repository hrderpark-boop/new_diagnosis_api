...
def _get_instruction_guide(
    ...
    if _next_name and _no_yield_forced:
        ...
        _ready_to_end_guide = (
            "🛑 **[무수확 강제 전환] 이 영역에서는 채점 가능한 구체적 실사례가 "
            "끝내 확보되지 않았습니다.** 없는 강점을 지어내 칭찬하지 마세요 "
            "(거짓 공감 금지).\n\n"
            "🎯 담백하고 정중하게 이 영역을 정리하되, '계속/휴식'을 묻지 말고 "
            "코치가 주도적으로 다음 영역으로 넘어감을 '선언'하세요. 선택권을 "
            "넘기면 지친 사용자는 그대로 이탈합니다.\n"
            "  예: '이 영역은 구체적인 사례까지는 오늘 충분히 담기 어려웠네요. "
            "그건 그것대로 괜찮습니다. 이 부분은 여기서 정리하고, 바로 다음 "
            f"'{_next_name}' 이야기로 이어가 보겠습니다 — {_next_name}와 "
            "관련해 최근 떠오르는 장면이 하나 있으실까요?'\n\n"
